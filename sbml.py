...
class FuncNode(Node):
    def __init__(self, funcname, params, block, output):
        self.funcname = funcname
        self.params = params
        self.block = block
        self.output = output
        var_table[self.funcname.value] = self
...
class FunctionCall(Node):
    ...
    def evaluate(self):
        import copy
        # print("in func call evaluate")
        global var_table
        fun_node = var_table[self.name.value]
        old_env = copy.deepcopy(var_table)
        for i in range(len(self.args)):
            var_table[fun_node.params[i].value] = self.args[i].evaluate()
        fun_node.block.evaluate()
        result = fun_node.output.evaluate()
        var_table = old_env
        return result
...
class ProgramNode(Node):
    ...
    def evaluate(self):
        # print("in program node evaluate")
        for func in self.funcs:
            func_dic[func.funcname.value] = func     # func.funcname
        self.block.evaluate()
...
def p_func(p):
    '''func : FUN VARIABLE LPAREN params RPAREN EQUALS block expression SEMICOLON'''
    p[0] = FuncNode(p[2], p[4], p[7], p[8])
    # func_dic[p[2]] = p[0]
